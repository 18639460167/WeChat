...
-(void)setDataSource:(NSArray *)dataSource{
    ...
    for (NSUInteger i=0; i<dataSource.count; i++)
    {
        ...
        {
            [iv sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",dataSource[i]]] placeholderImage:PLACE_IMAGE];
        }
        else if ([dataSource[i] isKindOfClass:[NSURL class]])
        {
            [iv sd_setImageWithURL:dataSource[i] placeholderImage:PLACE_IMAGE];
        }
